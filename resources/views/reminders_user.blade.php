...
<head>
    ...
    <title id="reminder">Reminder</title>
...
<div class="container" style="margin-top: 50px;">
    ...
    <h4 class="text-center" id="reminder2">Reminder</h4><br>

    <h5># Tambah Reminder</h5>
    <div class="card card-default">
        <div class="card-body">
            <form id="addCustomer" class="form-inline" method="POST" action="">
                <div class="form-group mb-2">
                    <label for="name" class="sr-only">Tanggal</label>
                    <input id="name" type="date" class="form-control" name="name" placeholder="Tanggal"
                           required autofocus>
                </div>
                <div class="form-group mx-sm-3 mb-2">
                    <label for="number" class="sr-only">Deskripsi</label>
                    <textarea name="price" id="price" placeholder="Deskripsi" required autofocus></textarea>
                </div>
...
<script>
    ...
    var fullName = firebase.database().ref('user/{{$id}}/fullName');
    fullName.once('value', function(snapshot) {
        var value = snapshot.val();
        var htmls_reminder = [];
        htmls_reminder.push('Reminder ' + value);
        $('#reminder').html(htmls_reminder);
        $('#reminder2').html(htmls_reminder);
    });

    // Get Data
    firebase.database().ref('user/{{$id}}/reminders').on('value', function (snapshot) {
        var value = snapshot.val();
        var htmls = [];
    ...
    $('body').on('click', '.updateData', function () {
        ...
        firebase.database().ref('user/{{$id}}/reminders/' + updateID).on('value', function (snapshot) {
            var values = snapshot.val();
            var updateData = '<div class="form-group">\
    		        <label for="first_name" class="col-md-12 col-form-label">Nama</label>\
    		        <div class="col-md-12">\
    		            <input id="first_name" type="date" class="form-control" name="name" value="' + values.date + '" required autofocus>\
    		        </div>\
    		    </div>\
    		    <div class="form-group">\
    		        <label for="last_name" class="col-md-12 col-form-label">Price</label>\
    		        <div class="col-md-12">\
                    <textarea name="last_name" id="last_name" placeholder="Deskripsi" required autofocus>' + values.description + '</textarea>\
    		        </div>\
    		    </div>';
    ...
    $('.updateCustomer').on('click', function () {
        ...
        var updates = {};
        updates['/user/{{$id}}/reminders/' + updateID] = postData;
